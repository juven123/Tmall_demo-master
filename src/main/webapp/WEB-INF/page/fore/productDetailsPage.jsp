    ...
    <div class="context_info">
        ...
        <dl class="context_info_member">
            ...
            <dd>
                ...
                <span class="goods_price"> <em>库存${requestScope.product.product_keep_sum}件</em></span>
            </dd>
        </dl>

             <tr>
                <td>秒杀开始时间</td><td>${product.product_start_time}</td>
                <span id="skillTip">  <input type="hidden" id="remainSeconds"  value="${remainSeconds}">
                    <c:choose>
                        <c:when test="${secKillStatus==0}">
                            <span>秒杀倒计时:<span><span id ="countDown" >${remainSeconds}</span>秒</span>
                        </c:when>
                        <c:when test="${secKillStatus==1}">
                             <span>秒杀中</span>
                        </c:when>
                        <c:otherwise>
                            <span>秒杀结束</span>
